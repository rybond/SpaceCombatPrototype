...
func take_damage(amount: int):
	...
	if current_health <= 0:
		var my_arena = get_parent()
		call_deferred("die", my_arena)

func die(my_arena: Node):
	split(my_arena)
	destroyed.emit()
	queue_free()

func split(my_arena: Node):
	if child_scene == null or my_arena == null:
		return
	...
	for i in range(num_children):
		...
		my_arena.add_child(child)
		my_arena.register_asteroid( child)
